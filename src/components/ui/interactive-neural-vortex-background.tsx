  ...
  useEffect(() => {
    ...
    const fsSource = `
      ...
      void main() {
        ...
        color = mix(color, vec3(0.02, 0.15, 0.3), 0.32 + 0.16 * sin(2.0 * u_scroll_progress + 1.2));
        color += vec3(0.05, 0.0, 0.2) * sin(2.0 * u_scroll_progress + 1.5);
        color = color * noise;
    ...
    const render = () => {
      ...
      gl.uniform2f(uPointer, pointer.current.x / window.innerWidth, 1 - pointer.current.y / window.innerHeight);
